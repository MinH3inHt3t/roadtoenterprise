<template>
	<div class="home">
		...
		<Permission :roles="['logged-in']" @access-check="onCheck">
			...
			<div class="space-y-3">
				<div class="shadow border p-3 flex justify-between" v-for="comment of comments" :key="comment.id">
					<span>{{ comment.message }}</span>
					<!-- Check if a user should be able to see Edit link -->
					<Permission :roles="['owner', 'moderator', 'admin']" :entityOwnerId="comment.authorId">
						<a>Edit</a>
					</Permission>
				</div>
			</div>
